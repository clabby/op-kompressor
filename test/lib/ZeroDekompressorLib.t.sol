...
/// @dev Tests for the `ZeroDekompressorLib.sol` contract.
contract ZeroDekompressorLib_Sol_Test is FFIHarness {
...
/// @dev Tests for the `ZeroDekompressorLib.huff` contract.
contract ZeroDekompressorLib_Huff_Test is FFIHarness {
    MockDekompressor mockDekompressor;

    error InvalidInput();

    function setUp() public {
        // Deploy a new `MockDekompressor.huff`
        string[] memory commands = new string[](3);
        commands[0] = "huffc";
        commands[1] = "-b";
        commands[2] = "./test/mocks/MockDekompressor.huff";
        bytes memory code = vm.ffi(commands);
        assembly {
            sstore(mockDekompressor.slot, create(0x00, add(code, 0x20), mload(code)))
        }
    }
